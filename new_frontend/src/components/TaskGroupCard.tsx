...
function DraggableTaskItem({ task, index, isSingleTask, onClick }: { task: Task; index: number; isSingleTask: boolean; onClick: () => void }) {
  ...
  return (
    <div
      ...
      className={`p-4 cursor-move hover:bg-gray-50 ${index > 0 ? 'border-t border-gray-100' : ''}`}
      onClick={onClick}
    >
...
export default function TaskGroupCard({ tasks, emailInfo, onClick }: TaskGroupCardProps) {
  ...
  return (
    <Card className="overflow-hidden hover:shadow-md transition-shadow">
      ...
      {/* Tasks - Each task is individually draggable */}
